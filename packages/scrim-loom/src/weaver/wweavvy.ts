...
import { heddles, Management } from '../index.js';
...
export class Weavvy extends Weaver {
   ...
   */
  async buildPlan(workspace: any, warp: any): Promise<WeavingPlan & { _scrimValidated?: boolean }> {
    ...
    try {
      ...
      throw this.ferror(error as Error, {
        ...
        suggestions: [
          {
            action: 'view-dag',
            message: 'Check AAAArchi.buildProjectDAG() for architecture visualization'
          },
          { action: 'fix-warp', message: 'Update loom/WARP.ts to fix architectural violations' },
          {
            action: 'disable-validation',
            message: 'Set validateArchitecture: false to skip (not recommended)'
          }
        ],
  ...
  async weave(options?: { saga?: WeavvyConfig['saga'] }): Promise<WeavingResult> {
    ...
    try {
      ...
      // Step 2: Enrichment
      const heddlesResult = heddles.enrich({
      ...
      for (let i = executedSteps.length - 1; i >= 0; i--) {
        ...
        if (step.name === 'generation' && generationResult) {
          console.log(
            `🦡 Weavvy: Compensation - would delete ${generationResult.filesGenerated} files`
          );
    ...
    return (
      generationResult ?? { filesGenerated: 0, filesModified: 0, filesUnchanged: 0, errors: [] }
    );
  }
   ...
   */
  validateManagement(mgmt: Management): void {
    const enriched = heddles.enrich(mgmt);
    heddles.throwIfErrors(enriched);
  }
}
