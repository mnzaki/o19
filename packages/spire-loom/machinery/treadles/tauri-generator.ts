...
export const tauriPluginTreadle: PlatformWrapperTreadle = definePlatformWrapperTreadle({
  platform: {
    name: 'Tauri',
    spiraler: 'TauriSpiraler',
  },
  wrapperType: 'tauri-plugin',
  
  // TauriSpiraler matches with AndroidSpiraler and DesktopSpiraler (mux pattern)
  // This overrides the default match pattern (which would be RustCore)
  matches: [
...
export async function generateTauriPlugin(
  ...
  const commandNames = files
    .filter(f => f.path.includes('commands.rs'))
    .flatMap(() => []); // TODO: extract command names from generated content
