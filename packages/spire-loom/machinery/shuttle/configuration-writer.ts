...
import { ensureDir, readJson, writeJson } from '../sley/file-system-operations.js';
 ...
 */
export function ensureTauriPermissions(cratePath: string, permissions: TauriPermission[]): void {
 ...
 */
export function ensureBuildRs(cratePath: string, content: string): boolean {
 ...
 */
export function ensureBuildDependency(cratePath: string, name: string, version = '*'): boolean {
  ...
  if (hasBuildDeps) {
    ...
    const newContent = content.replace(/(\[build-dependencies\]\n)/, `$1${depLine}\n`);
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android">
  ...
  const newContent = content.replace(/(\s*<\/application>)/, `\n${serviceElement}$1`);
