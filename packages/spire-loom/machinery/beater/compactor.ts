...
export abstract class Compactor {
  ...
  async compact(): Promise<CompactorResult> {
    ...
    try {
      ...
      throw new Error(`🔧 CRINKLE! Compaction failed:\n${(error as Error).message}`);
    }
...
export function defineCompactor(spec: CompactorSpec): new (config: CompactorConfig) => Compactor {
  return class extends Compactor {
    protected generate() {
      return spec.generate(this.config.midstagePath, this.config.outputPath);
    }
    protected compile() {
      return spec.compile(this.config.midstagePath);
    }
    protected run() {
      return spec.run(this.config.midstagePath, this.config.outputPath);
    }
    protected parse(stdout: string) {
      return spec.parse(stdout);
    }
    protected verify(files: string[]) {
      return spec.verify?.(files) || Promise.resolve();
    }
  };
}
