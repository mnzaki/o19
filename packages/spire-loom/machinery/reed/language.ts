...
import { declare, getScopeRegistry, type Scope } from '../self-declarer.js';
...
export type { CrudOperation, MethodLink } from './language-types.js';
...
export const declareLanguage = declare<LanguageDefinition, LanguageDefinition>({
  ...
  declare: (def) => {
    ...
    if (def.codeGen.typeMappings?.length) {
      queueMicrotask(() => {
        registerLanguageTypeMappings(def.name, def.codeGen.typeMappings!).catch((err) =>
          console.warn('[language] Failed to register type mappings:', err)
        );
      });
    }
