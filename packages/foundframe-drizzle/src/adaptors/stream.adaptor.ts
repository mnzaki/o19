...
import { thestream, person, post, media, bookmark, conversation } from '../schema/index.js';
...
export class DrizzleStreamAdaptor extends StreamAdaptor implements StreamPort {
  ...
  async addChunk(type: StreamChunkType, entityId: number, seenAt?: Date): Promise<StreamEntry> {
    ...
    switch (type) {
      case 'person':
        insertData = { ...baseData, personId: entityId, postId: null, mediaId: null, bookmarkId: null, conversationId: null };
        break;
      case 'post':
        insertData = { ...baseData, personId: null, postId: entityId, mediaId: null, bookmarkId: null, conversationId: null };
        break;
      case 'media':
        insertData = { ...baseData, personId: null, postId: null, mediaId: entityId, bookmarkId: null, conversationId: null };
        break;
      case 'bookmark':
        insertData = { ...baseData, personId: null, postId: null, mediaId: null, bookmarkId: entityId, conversationId: null };
        break;
      case 'conversation':
        insertData = { ...baseData, personId: null, postId: null, mediaId: null, bookmarkId: null, conversationId: entityId };
        break;
    }
  ...
  private async hydrateEntry(row: typeof thestream.$inferSelect): Promise<StreamEntry | null> {
    ...
    if (row.personId) {
      ...
      const b = await this.db.select().from(bookmark).where(eq(bookmark.id, row.bookmarkId)).limit(1);
      if (b.length > 0) {
        chunk = {
          type: 'bookmark',
          id: b[0].id,
          entity: {
            ...
            creationContext: b[0].creationContext as { browsingHistory: string[]; referrer?: string; timestamp: number },
            createdAt: b[0].createdAt,
          }
        };
      }
    } else if (row.conversationId) {
      const c = await this.db.select().from(conversation).where(eq(conversation.id, row.conversationId)).limit(1);
      if (c.length > 0) {
        chunk = {
          type: 'conversation',
          id: c[0].id,
