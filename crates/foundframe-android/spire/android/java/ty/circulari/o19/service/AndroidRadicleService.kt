...
import ty.circulari.o19.IAndroidRadicle

/**
 * AndroidRadicleService - Generated foreground service with AIDL IPC
 * 
 ...
 */
class AndroidRadicleService : Service() {
    
    companion object {
        private const val TAG = "ANDROIDRADICLESERVICE"
        private const val NOTIFICATION_ID = 1
        private const val CHANNEL_ID = "androidradicleservice"
        
        init {
            try {
                Log.i(TAG, "[AndroidRadicleService] Loading native library...")
                System.loadLibrary("android")
                Log.i(TAG, "[AndroidRadicleService] Native library loaded successfully")
            } catch (e: Throwable) {
                Log.e(TAG, "[AndroidRadicleService] FAILED to load native library: ${e.message}", e)
            }
        }
    }
     ...
     */
    private val aidlBinder = object : IAndroidRadicle.Stub() {
    ...
    override fun onCreate() {
        super.onCreate()
        Log.i(TAG, "[AndroidRadicleService] Service creating in pid ${Process.myPid()}")
        
        // Create notification channel for foreground service
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                CHANNEL_ID,
                "AndroidRadicleService",
                NotificationManager.IMPORTANCE_LOW
            ).apply {
                description = "Background service for android"
                setShowBadge(false)
            }
        ...
        Log.i(TAG, "[AndroidRadicleService] Started as foreground service")
    }
    
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        Log.i(TAG, "[AndroidRadicleService] Service starting (startId=$startId)")
        
        val homeDir = getDir(".android", Context.MODE_PRIVATE).absolutePath
        ...
        Thread {
            try {
                ...
                if (nativeHandle < 0) {
                    Log.e(TAG, "[AndroidRadicleService] Native service failed to start")
                    stopSelf(startId)
                    return@Thread
                }
                
                Log.i(TAG, "[AndroidRadicleService] Native service started with handle=$nativeHandle")
                
                // Service keeps running until explicitly stopped
                // The Rust core runs its own event loop
                Thread.currentThread().join()
                
                Log.w(TAG, "[AndroidRadicleService] Native service thread exited unexpectedly")
            } catch (e: Exception) {
                Log.e(TAG, "[AndroidRadicleService] Native service crashed: ${e.message}", e)
            } finally {
                if (nativeHandle != 0L) {
            ...
            name = "AndroidRadicleService-Thread-$startId"
            start()
        }
    ...
    override fun onBind(intent: Intent): IBinder {
        Log.i(TAG, "[AndroidRadicleService] onBind() - returning AIDL binder")
        return aidlBinder
    }
    
    override fun onDestroy() {
        super.onDestroy()
        Log.i(TAG, "[AndroidRadicleService] Service destroying")
        if (nativeHandle != 0L) {
            nativeStopService(nativeHandle)
            nativeHandle = 0
        }
