...
// 
// Template: /home/mnzaki/Projects/circulari.ty/o19/packages/spire-loom/machinery/bobbin/tauri/desktop_mainline_bridge.rs.mejs
// Template file: desktop_mainline_bridge.rs.mejs
// To override: Copy this template to loom/bobbin/tauri/desktop_mainline_bridge.rs.mejs
// 
...
// Use types from main crate::models (not spire::models) to match Platform trait
use crate::models::{NotificationPermissionStatus, PairingQrResponse, ScannedPairingData, PairedDeviceInfo};
...
impl<R: Runtime> Platform for SpireFoundframeDesktopBridge<R> {
  ...
  fn request_permissions(&self) -> Result<NotificationPermissionStatus> {
    // Convert from spire::models::NotificationPermissionStatus to crate::models::NotificationPermissionStatus
    let spire_status = self.spire_platform.request_permissions()
      .map_err(|e| Error::Other(e.to_string()))?;
    Ok(NotificationPermissionStatus {
      status: spire_status.status,
    })
  }
  ...
  fn generate_pairing_qr(&self, _device_name: String) -> Result<crate::models::PairingQrResponse> {
    Err(Error::Other("generate_pairing_qr not implemented in bridge platform".into()))
  }

  fn parse_pairing_url(&self, _url: String) -> Result<crate::models::ScannedPairingData> {
    Err(Error::Other("parse_pairing_url not implemented in bridge platform".into()))
  }

  fn check_followers_and_pair(&self) -> Result<Vec<crate::models::PairedDeviceInfo>> {
...
/// 
/// Note: Trait-based access (BookmarkOps, DeviceOps, etc.) not yet implemented.
/// Use `spire_platform()` for direct access to operations.
pub trait SpireBridgeExt<R: Runtime> {
  /// Access the spire platform for direct CRUD operations.
  fn spire_platform(&self) -> &impl SpireFoundframePlatformTrait;
}

impl<R: Runtime> SpireBridgeExt<R> for SpireFoundframeDesktopBridge<R> {
  fn spire_platform(&self) -> &impl SpireFoundframePlatformTrait {
    &self.spire_platform
  }
}
