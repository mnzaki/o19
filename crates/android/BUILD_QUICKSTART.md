...
This guide shows how to build the `android` package (including the Rust `FoundframeRadicle` service) in a separate terminal.
...
```bash
cd /path/to/circulari.ty/o19/packages/android
./build-rust.sh
```
...
```
src/main/jniLibs/
├── arm64-v8a/libandroid.so
├── armeabi-v7a/libandroid.so
└── x86_64/libandroid.so
```
