...
// Re-export the three "skools" of encoding
pub use reversible::{
    Skool, 
    CHAR_COUNT_256,
    // New skool-based API
    encode_256, 
    decode_256,
    // Cjkskool-specific sets
    CJK_SET, BRAILLE_SET, ARROWS_SET, GEOMETRIC_SET, NUMBER_FORMS_SET, LETTERLIKE_SET,
};
...
impl EmojiIdentity {
  ...
  /// Create from 256-bit entropy using the three "skools":
  ...
  ///
  /// The skool is determined by the `preferred_skool` parameter. If None,
  /// auto-detects based on entropy bits.
  pub fn from_256_bits_skool(entropy: [u8; 32], preferred_skool: Option<reversible::Skool>) -> Self {
    let (string, config, _skool) = reversible::encode_256(&entropy, preferred_skool);
    Self {
      string,
      config,
      entropy: entropy.to_vec(),
  ...
  /// Create using oldskool (original irreversible, kept for history).
  pub fn from_256_bits_oldskool(entropy: [u8; 32]) -> Self {
    Self::from_256_bits_skool(entropy, Some(reversible::Skool::Oldskool))
  }

  /// Create using newskool (reversible emoji + symbols).
  pub fn from_256_bits_newskool(entropy: [u8; 32]) -> Self {
    Self::from_256_bits_skool(entropy, Some(reversible::Skool::Newskool))
  }

  /// Create using cjkskool (reversible CJK + Braille + Arrows + etc.).
  pub fn from_256_bits_cjkskool(entropy: [u8; 32]) -> Self {
    Self::from_256_bits_skool(entropy, Some(reversible::Skool::Cjkskool))
  }

  /// Decode this emoji identity back to 256-bit entropy.
  ///
  /// Returns `Some([u8; 32])` if the string was encoded reversibly,
  /// `None` if decoding fails (oldskool is irreversible).
  pub fn to_entropy(&self) -> Option<[u8; 32]> {
    reversible::decode_256(&self.string).map(|(entropy, _config, _skool)| entropy)
  }

  /// Attempt to decode an emoji string to entropy.
  ///
  /// Auto-detects the skool from the first character.
  pub fn decode_string(emoji_string: &str) -> Option<[u8; 32]> {
    reversible::decode_256(emoji_string).map(|(entropy, _config, _skool)| entropy)
  }
}
