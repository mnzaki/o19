...
This document explains how to build the `android-activities` package, which includes the Rust `FoundframeRadicle` service and the Kotlin Android wrapper.
...
```bash
cd o19/packages/android-activities
./build-rust.sh
```
...
3. Packages into AAR: `build/outputs/aar/android-activities-release.aar`
...
src/main/jniLibs/
├── arm64-v8a/
│   └── libandroid_activities.so      # Main Rust library
├── armeabi-v7a/
│   └── libandroid_activities.so
└── x86_64/
    └── libandroid_activities.so

build/outputs/aar/
├── android-activities-release.aar     # Release AAR
└── android-activities-debug.aar       # Debug AAR
```
...
```gradle
dependencies {
    implementation files('libs/android-activities-release.aar')
}
```
