...
pub fn execute_sql(db_path: &Path, query: SqlQuery) -> crate::Result<Vec<SqlRow>> {
  ...
  for row_result in stmt.into_iter() {
    let row = row_result.map_err(|e| crate::Error::Sql(e.to_string()))?;
...
/// Read all values from a row using known column count
...
fn read_value_at(row: &sqlite::Row, index: usize) -> crate::Result<Value> {
  ...
  if let Ok(v) = row.try_read::<&[u8], _>(index) {
    ...
    return Ok(Value::String(STANDARD.encode(v)));
  }
...
mod tests {
  ...
  fn test_execute_sql() {
    ...
    // INSERT returns empty result set
    assert!(result.is_empty());
    ...
    let row = &result[0];
    assert_eq!(row.values.len(), 3); // id, name, active
