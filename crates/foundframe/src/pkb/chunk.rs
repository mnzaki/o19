  ...
  pub fn ingest(&self, dir_path: &Path, relative_path: &Path) -> Result<EntryId> {
    ...
    match self {
      ...
      StreamChunk::TextNote { content, title } => {
        let entry = super::entry::Entry::from_text_note(content.clone(), title.clone());
        entry.write_to(&full_path)?;
      }
      StreamChunk::StructuredData { db_type, data } => {
        let entry = super::entry::Entry::from_structured_data(db_type.clone(), data.clone());
        entry.write_to(&full_path)?;
      }
    }
...
pub mod heuristics {
  ...
  pub fn guess_directory(chunk: &StreamChunk) -> Vec<(&'static str, f32)> {
    ...
    match chunk {
      ...
      StreamChunk::StructuredData { db_type, .. } => match db_type.as_str() {
...
mod tests {
  ...
  fn test_chunk_filename() {
    ...
    assert_eq!(
      chunk.generate_filename(1234567890, Some("My Photo")),
      "1234567890 My Photo.mln"
    );
    ...
    assert_eq!(
      chunk2.generate_filename(1234567890, None),
      "1234567890.js.md"
    );
  }
