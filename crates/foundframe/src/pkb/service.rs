...
use super::{
    ...
    directory::{Directory, DirectoryMeta, DirectoryRegistry, DirectoryEntry, PkbBase},
    entry::Entry,
    merge::MergeResult,
    now_timestamp,
};
...
impl PkbService {
    ...
    pub fn create_directory(
        ...
        if description.len() < 16 {
            return Err(Error::Other(
                format!("Description must be at least 16 characters, got {}", description.len())
            ));
        }
        ...
        let (rid, _doc, _signed_refs) = radicle::rad::init(
            ...
            project_name,
            description,
            git::BranchName::try_from("main")
                .map_err(|_| Error::Other("Invalid branch name".into()))?,
            visibility,
            profile.signer().map_err(|e| Error::Other(e.to_string()))?.as_ref(),
    ...
    pub fn sync_device_remotes(&mut self, directory: &str) -> Result<()> {
        let dir = self.get_directory(directory)?
    ...
    pub fn add_device_remote(
        ...
        // Get device info
        let device = self.devices.get(device_nid)?
    ...
    pub fn sync_directory(&mut self, name: &str) -> Result<MergeResult> {
        let dir = self.get_directory(name)?
        ...
        for path in &merge_result.added {
            ...
            if let Ok(content) = std::fs::read(&full_path) {
                ...
                let source_device = fetched_from.first().cloned().unwrap_or_else(|| "unknown".into());
