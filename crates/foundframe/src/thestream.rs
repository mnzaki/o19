...
impl TheStream {
    ...
    fn dir_path(&self, directory: &DirectoryId) -> Result<std::path::PathBuf> {
        ...
        Ok(std::path::PathBuf::from(format!("{}/dirs/{}", 
        ...
        match event {
            PkbEvent::EntryCreatedOrPulled { 
                ...
                let reference = format!(
                    "pkb://{}/{}/{}",
                    identity.string, directory, path.display()
                );
            ...
            PkbEvent::SyncStarted { directory } => {
                Some(TheStreamEvent::SyncStarted { 
                    directory: directory.clone() 
                })
            }
            ...
            PkbEvent::SyncFailed { directory, error } => {
                Some(TheStreamEvent::SyncFailed {
                    directory: directory.clone(),
                    error: error.clone(),
                })
            }
    ...
    pub fn add_text_note(
        ...
        let filename = chunk.generate_filename(
            crate::pkb::now_timestamp(),
            title,
        );
    ...
    fn extract_summary(chunk: &StreamChunk) -> Option<StreamSummary> {
        match chunk {
            StreamChunk::MediaLink { url, title, .. } => {
                Some(StreamSummary {
